...
$(document).on('click', ".btn-addit-quiz-list", function(){
  ...
  var drag_handler = $("#quiz_list_popup_drag")
...
// 트위터 체크 추가
$(document).on("click", "#checkbox_friend_twiter_url" ,function(){
    bchecked = $(this).is(":checked");
    toggelCheckBox(bchecked, $(this).parent().siblings(), "disabled");
    $(this).parent().siblings().focus();
});
// 유튜브 영상 사용 체크
$(document).on("click", "#checkbox_youtube_id" ,function(){
    bchecked = $(this).is(":checked");
    toggelCheckBox(bchecked, $(this).parent().siblings(), "disabled");
    $(this).parent().siblings().focus();
});
// sns뷰 사용 체크
// 인증샷 체크박스 클릭
$(document).on("click", "#checkbox_sns_view" ,function(){
    bchecked = $(this).is(":checked");
    toggelCheckBox(bchecked, "#sns_view_count", "disabled");
    $('#sns_view_count').focus();
});

// 2023.05.15 추가 파일 다운로드 사용체크
$(document).on("click", "#checkbox_file_down" ,function(){
    bchecked = $(this).is(":checked");
    toggelCheckBox(bchecked, "#file_down_count", "disabled");
    $('#file_down_coun').focus();
});
...
// 회원가입버튼 클릭
$(document).on('click', '#btn_signIn_popup_open', function(){
    $("#signIn_popup").fadeIn();
    $("html, body").addClass("not_scroll");
});
$(document).on('click', '#btn_signIn_popup_close', function(){
...
//회원가입 알람 팝업버튼
$(document).on('click', '#btn_alarm_popup', function(){
    $("#signIn_alarm_popup").fadeIn();
    $("html, body").addClass("not_scroll");
});
$(document).on('click', '#btn_alarm_popup_close', function(){
    $("#signIn_alarm_popup").fadeOut();
    $("html, body").removeClass("not_scroll");
    posY = $(window).scrollTop();
});
...
$(document).on('click', '#btn_result_select', function(){
  ...
  var nAnswer = $("#txt_random_answer").val();
  ...
  if(Number(nCount) > global_event_result_json.tabledata.length){
    alert("추첨인원이 참여인원보다 많습니다")
    return
  }
  if(!isEmpty(nAnswer)){
    var randomAnsweChkArrat = randomAnswer(nAnswer, global_event_result_json.tabledata, global_event_result_json.tabledata.length);
    if(Number(nCount) > randomAnsweChkArrat.length){
        alert("해당 값을 작성한 인원이 추첨인원보다 적습니다.")
        return;
    }
    var randomArray = generateRandom(parseInt(nCount), randomAnsweChkArrat.length);
    draw_random_list2(randomAnsweChkArrat, randomArray, '#random_list_view');
  }else{
...
// 랜덤추첨 사용자입력값 호버시
$(document).on('mouseover', '.random-answer', function(){
    var text = $(this).text()
    $("#random_answer_view").text(text);
    $("#random_answer_view").css('display','block');
});
$(document).on('mouseout', '.random-answer', function(){
    
    $("#random_answer_view").css('display','none');
});
...
$(document).on('click', '#btn_join_event', function(){
    ...
    let nowdate = getNewDate();
    nowdate = nowdate.replaceAll('-', '')
    event_start_day = get_string_length(event_start_day, 0, 10)
    event_start_day = event_start_day.replaceAll('-','')
    event_end_day = get_string_length(event_end_day, 0, 10)
    event_end_day = event_end_day.replaceAll('-','')

    // 날짜비교
    if(Number(nowdate) - Number(event_start_day) < 0){
        Swal.fire({
            title: "알림",
            text: '아직 이벤트 기간이 아닙니다.',
            icon: 'error',
            width: 400,
        ...
        return;
    }else if(Number(nowdate) - Number(event_end_day) > 0){
        Swal.fire({
            title: "알림",
            text: '이벤트기간이 지났습니다.',
            icon: 'error',
    ...
    if ($(rtnIDString(privacy_address)).length) {
        if (isEmptyMsg(TrimData(rtnIDString(privacy_address)), "주소를 입력하세요.")){ 
            $(rtnIDString(privacy_address)).focus();
            return 
        };
    }
    ...
    */
        ...
        $.each(tmpJSON, function(key, items){   
            ...
            if (items == "checkbox"){           
                ...
                for (var i=0; i<tmpIDArray.length; i++){
                    ...
                    var tmpTxID = rtnIDString(tmpIDArray[i] + "_text");
                    if ($(tmpID).is(':checked')){
                        if(isEmpty($(tmpTxID).val())){
                            chk_arr.push($(tmpID).val());
                        }else{
                            chk_arr.push($(tmpID).val() + "$" + $(tmpTxID).val());
                        }
                ...
                let radioText = $("input[id='" + radiocheckedID +"']").val();
                var subTxetVal = $(`#${radiocheckedID}_text`).val();
                // 정답이 공백이 아니면 결과를 구한다.
                ...
                if(isEmpty(subTxetVal)){
                    userAnswerJSON['user-answer'] = radioText;
                }else{
                    userAnswerJSON['user-answer'] = radioText + "$" + subTxetVal;
                }
    ...
    let user_address = TrimData(rtnIDString(privacy_address));
    ...
    if (nUploadFileCount > 0){
        ...
        if (szUpload === "success"){
            ...
            pParms.user_address = user_address;
...
$(document).on('click', "#event_popupo_next", function(){
    $(".layer-popup").css({ visibility: " hidden" , opacity : "0" });
    $(".layer-popup .popup_content").css({ display: "none" });
    // goToPage('index.html', '');
});

$(document).on('click', "#ox_popupo_next", function(){
    $(".layer-popup").css({ visibility: " hidden" , opacity : "0" });
    $(".layer-popup .popup_content").css({ display: "none" });
});
...
$(document).on("click", ".addit-page-a", function(){
    ...
    params.searchtype = "";         // 검색 유형
    params.searchdata = "";         // 검색 데이터
    if($("#btn_event_list_active").hasClass("active") === true){
        active = "1";
    }else if($("#btn_event_list_prev").hasClass("active") === true){
        active = "0";
    }else if($("#btn_event_list_end").hasClass("active") === true){
        active = "0";
    }
    ...
    switch(type){
        case "event":
            params.cmd = "load_event";
            params.e_active = active;
            global_event_list_json = load_data(params);
            // goToPage('index.html', pages);
            if($("#btn_event_list_prev").hasClass("active") === true) {
                draw_event_list_prev(global_event_list_json, '#event_list', "#page_div", nowpage);       
            }else if($("#btn_event_list_end").hasClass("active") === true){
                draw_event_list_end(global_event_list_json, '#event_list', "#page_div", nowpage);
            }else{
                draw_event_list(global_event_list_json, '#event_list', "#page_div", nowpage);
            }
...
// sns뷰 버튼 버튼 클릭
$(document).on('click', "#btn_sns_view_count", function(){
    var count = parseInt(TrimData('#sns_view_count'));
    if (!fn_isNumeric(count, '+')){
        alert('숫자(정수값, 양수)만 입력해야 합니다.');
        return;
    }
    DrawSnsView(count, "#sns_view_print");
});


// 파일 다운로드 버튼 클릭
$(document).on('click', "#btn_file_down_count", function(){
    var count = parseInt(TrimData('#file_down_count'));
    if (!fn_isNumeric(count, '+')){
        alert('숫자(정수값, 양수)만 입력해야 합니다.');
        return;
    }
    DrawFileDown(count, "#file_down_print");
});
...
$(document).on('click', ".shot-class", function(){
    ...
    {
        $(this).siblings('.shot-url').css("display", "none");
    }
    else if(szType == "2"){                       // 댓글 인증샷일 경우
        $(this).siblings('.shot-url').css("display", "none");
    }else{
...
$(document).on('click', '#btn_event_modify', function(){
    ...
    {   
        ...
        if (!isEmpty(e_idx)){
          ...
          $("#e_idx").val(e_idx);
          global_evnet_json = load_data(params);
          EventModifyGo(global_evnet_json, szParam);
        }
    }
});

// 이벤트 복사버튼
$(document).on('click', '#btn_event_copy', function(){
    if (confirm("이벤트를 복사 하시겠습니까?"))
    {   
        var e_idx = $(this).data('e_idx');
        var copy = $(this).data('copy');
        var szParam = 'e_idx=' + e_idx;
        var szParam2 = 'copy=' + copy
        if (!isEmpty(e_idx)){
          let params = {};
          params.cmd = "load_event";
          params.e_idx = e_idx;
          $("#e_idx").val(e_idx);
          global_evnet_json = load_data(params);
          EventModifyGo(global_evnet_json, szParam, szParam2);
        }
    }
});
...
$(document).on('click', "#btn_event_makes", function(){
    ...
    //트위터 추가
    snsFriend_json[baseinfo_sns_friend_twiter] = TrimDataEncoding(rtnIDString(baseinfo_sns_friend_twiter));

    baseinfo_json[baseinfo_sns_friend] = snsFriend_json;

    // 유튜브 추가
    baseinfo_json[baseinfo_youtube_id] = TrimDataEncoding(rtnIDString(baseinfo_youtube_id));

    // sns뷰 값 받기 (받기 전 한번 더 숫자인지 유효성 검사)
    if ($('#checkbox_sns_view').is(":checked")){
        var snsViewcount = TrimData('#sns_view_count');
        if (!isEmpty(snsViewcount)){
            if (!fn_isNumeric(snsViewcount, '+')){
                alert('숫자(정수값, 양수)만 입력해야 합니다.');
                return;
            }
        }
        ...
        // sns뷰 정보를 배열에 입력
        var sns_view_array = [];
        for(var i = 0; i < parseInt(snsViewcount); i++){
            var tmpName = rtnIDString(sns_view_type) + String(i);
            tmpdata = TrimDataEncoding(tmpName);
            sns_view_array.push(tmpdata);
        }
        baseinfo_json[sns_view_type] = sns_view_array;
    }
    else{
        baseinfo_json[sns_view_type] = "";
    }


    //2023.05.15 파일 다운로드 값 받기
    if ($('#checkbox_file_down').is(":checked")){
        var fileDowncount = TrimData('#file_down_count');
        if (!isEmpty(fileDowncount)){
            if (!fn_isNumeric(fileDowncount, '+')){
                alert('숫자(정수값, 양수)만 입력해야 합니다.');
                return;
            }
        }

        // 2023.05.15 파일 다운로드 배열 입력
        var file_down_array = [];
        for(var i = 0; i < parseInt(fileDowncount); i++){
            var tmpName = rtnIDString(file_down_type) + String(i);
            tmpdata = TrimDataEncoding(tmpName);
            file_down_array.push(tmpdata);
        }
        baseinfo_json[file_down_type] = file_down_array;
    }
    else{
        baseinfo_json[file_down_type] = "";
    }
    ...
    if ($('#checkbox_event_prev_info').is(":checked")){
        ...
        if(quizType === "1"){
            ...
            for(var j = 0; j <nVeiwCount; j++){
                ...
                if (view_Json[QuizInfo_subVal_type] == "0"){ // 텍스트일 경우
                    var textChk = 'input:checkbox[name=' + QuizInfo_subVal_type +  + String(Problem_Array[i]) + String(j) + '_sub' + QuizInfo_subVal_data + '_1]:checked';
                    ...
                    view_Json["subTextView"] = TrimData(textChk);
    ...
    agreed_json[privacy_address] = getElementCheckValue(rtnIDString(privacy_address));
    if ($('#checkbox_privacy_customer').is(':checked')){
        var tmpID = rtnIDString(privacy_customer);
        var szTemp = TrimDataEncoding(tmpID);
        if (isEmpty(szTemp)){
            alert('고객사 방침을 추가하세요.');
            $(tmpID).focus();
            return false;
        }
        agreed_json[privacy_customer] = szTemp;
    }
...
$(document).on('click', '.btn-addit-quiz-list', function(){
  ...
  $('#table_td').children().each(function(index, tr){
    ...
    if(!isEmpty(string)){
        string = string[idx].split(",")
    }
    for(var i = 0; i <string.length; i++){
        tmpArray.push($.trim(string[i]));
    }
  ...
  // 챠크를 그린다.
  draw_chart(analyzeChart, chart_label_array, chart_data_array, '# 분석', $(this).siblings().html().split('<')[0]);
...
//   draw_list(analyzeChart, chart_label_array, chart_data_array, $(this).parent().html().split('<')[0], draw_target);
  draw_list_table(chart_label_array, chart_data_array, $(this).siblings().html().split('<')[0], draw_target, 'asc', tmpArray);
...
// 진행중 이벤트 버튼 클릭 시
$(document).on('click', '#btn_event_list_prev', function(){
    var params = {};
    params.e_active = "0";
    params.loadcount = -1;
    params.cmd = "load_event";
    global_event_list_json = load_data(params);
    draw_event_list_prev(global_event_list_json, '#event_list', "#page_div", 1);
...
$(document).on('click', '#btn_event_list_end', function(){
    ...
    params.cmd = "load_event";
    global_event_list_json = load_data(params);
    draw_event_list_end(global_event_list_json, '#event_list', "#page_div", 1);
...
$(document).on('click', '#btn_oxQuiz_makes', function(){
    ...
    for(var i = 0; i< parseInt(quizCount); i++){
        ...
        // 퀴즈 제목
        quiz_Json[QuizInfo_quizTitle] = TrimDataEncoding(rtnIDString(Quiz_Title + String(Problem_Array[i])));
        // 퀴즈 점수
    ...
    for(var i = 0; i< parseInt(resultCount); i++){
        ...
        var sztmpsrcID = rtnIDString(resultImg + "_img");
        if (isEmpty(tmpdata)){      // 메인이미지를 지정하지 않았을 경우
            // img src 엘리먼트는 공백이거나 null일 경우 baseURL을 리턴한다.
            var tmp = ($(sztmpsrcID).attr('src') === "") ? "" : $(sztmpsrcID).attr('src');
            if (isEmpty(tmp))
            {
                alert("결과 이미지는 반드시 지정해야 합니다.")
...
//ox 퀴즈 다음 버튼 클릭시 
// 이벤트 제안서를 위해 입력값을 받지 않습니다
$(document).on('click', '#btn_next', function(){
    if (g_debug = "1"){
        //이벤트 참가자 정보 체크   //테스트 서버에서 제안서 제출로 김이사님 요청으로 기능 정지 시켰습니다.(2023.08.29)
        // if ($(rtnIDString(privacy_name)).length) {
        //     if (isEmptyMsg(TrimData(rtnIDString(privacy_name)), "이름을 입력하세요.")){ 
        //         $(rtnIDString(privacy_name)).focus();
        //         return 
    ...
    // }
    swiper.slideNext();
    $(".addit-privacy-slide").css('height','100%')
    $(".StartDiv").css({'height':'auto','overflow':'hidden'})
});
...
// 이벤트 시작 버튼 호버시
$(document).on('mouseover', '.addit-StartButton', function(){
    $(".addit-StartButton").addClass('active')
});
$(document).on('mouseout', '.addit-StartButton', function(){
    $(".addit-StartButton").removeClass('active')
});

// ox이미지 클릭시
$(document).on('click', '.Quiz_anwer', function(){
    $(this).addClass('active');
    $(this).parent().siblings(".Quizcontent").children().addClass("UnActive");
});
...
$(document).on('click', '.last-contents', function(){
    ...
    setTimeout(function () {  
        ...
        for(var i = 0; i < quizLength; i++){
            if(totalScore === ""){
                totalScore = 0 + Number(user_score[i]);
            }else{
                totalScore = totalScore + Number(user_score[i]);
            }
        }
        for(var i = 0; i < quizLength; i++){
            user_a = "";
            ...
            user_ans["user-answer"] = user_a
            user_ans_arr.push(user_ans)
        }
        for(var i = 0; i < user_score.length; i++){
            if(user_score[i] === "0"){
                user_score.splice(i, 1);
                i--;
            }
        }
        quizLength = user_score.length;
        ...
        if (nUploadFileCount > 0){
            ...
            if (szUpload === "success"){
                ...
                pParms.user_inpath = g_inpath;
                pParms.shot = shot_arr.join(",");
                pParms.resultArray = JSON.stringify(user_ans_arr);
                var sendResult = send_paramdata(pParms, false, false);
                if (sendResult === "success"){
...
let clickId = "";
let textID = "";
$(document).on("click",".addit-box-type-12",function(){
    var tmpID = $(this).data("id");
    var tmpTextId = tmpID + "_text";
    clickId = "#" + tmpID;
    textID = "#" + tmpTextId
    // bchecked = $(clickId).is(":checked");
    // toggelCheckBox(bchecked, $(textID), "disabled");
});
...
function downloadFile(filePath) {
    var link = document.createElement('a');
    link.href = filePath;
    link.download = filePath.substr(filePath.lastIndexOf('/') + 1); // 파일명으로 다운로드될 때 사용할 이름을 설정합니다.
    link.target = '_blank';
    document.body.appendChild(link);
    link.click();
...
// 첨부파일 전체 다운로드
$(document).on('click', '#btn_flie_download_all', function(){
    let linkArray = []
    $('#table_td').children().each(function(index, tr){
        var text = ''
        var tmpUserArray = $(tr).children().eq(7).find('a').attr('href');   // "/"로 구분
        text = tmpUserArray
        linkArray.push(text)
                // 선택한 index에 해당되는 답을 얻어온 후, 앞뒤 공백 제거
    });
    for (var i = 0; i < linkArray.length; i++) {
        downloadFile(linkArray[i]);
    }
});

// sns로그인 관련
$(document).on('click', '#btn_sns_login_go', function(){
    goToPage('snsLogin.html', '')
});
$(document).on('click', '#btn_kakao_login', function(){
    loginWithKakao()
});


// 네이버 로그인 버튼이 로드되면 실행될 콜백 함수
